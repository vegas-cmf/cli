 ...
 * @package Vegas\Cli
 */
class TaskAbstract extends \Phalcon\Cli\Task
{
    ...
    public function beforeExecuteRoute()
    {
        ...
        $this->args = $this->dispatcher->getParam('params', null, []);
    ...
    {
        return in_array('--' . self::HELP_OPTION, $args) || in_array('-' . self::HELP_SHORTOPTION, $args);
    }
    ...
    {
        $output = implode('', $this->outputBuffer);
        return trim($output);
    }
    ...
    protected function validate()
    {
        ...
        $args = $this->dispatcher->getParam('params', null, []);
        if (isset($this->actions[$this->actionName])) {
            $action = $this->actions[$this->actionName];
            return $action->validate($args);
        }
    }
}
